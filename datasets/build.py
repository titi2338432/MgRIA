...
def Get_user_gp(cla_dic=False,file_dir = r"D:\学习\研究生任务\子璐论文\code\code&data\数据集切割\sampledata" ,test = 10):
    ...
    df_train_sort['month'] = df_train_sort.createtime.apply(lambda row:row.month,1)
    df_train_sort['day'] = df_train_sort.createtime.apply(lambda row:row.day,1)
    df_train_sort['weekday'] = df_train_sort.createtime.apply(lambda row:row.weekday(),1)
    df_train_sort['hour'] = df_train_sort.createtime.apply(lambda row:row.hour,1)
